...
<a
	...
	class="group flex flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white transition hover:-translate-y-0.5 hover:shadow-md"
>
	<div class="relative aspect-square w-full overflow-hidden bg-zinc-100">
		...
		{#if product.stock <= 0}
			<span
				class="absolute top-2 left-2 rounded-full bg-zinc-900/80 px-2 py-0.5 text-xs font-medium text-white"
			>
				Stok Habis
			</span>
		{/if}
