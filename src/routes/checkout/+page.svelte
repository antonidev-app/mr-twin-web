...
<div class="mx-auto max-w-4xl px-4 py-6">
	<h1 class="mb-6 text-2xl font-bold tracking-tight text-zinc-900">Checkout</h1>
	...
	<div class="flex flex-col gap-6 md:flex-row">
		<form id="checkout-form" onsubmit={submit} class="order-2 flex-1 space-y-4 md:order-1">
			...
			<button
				type="submit"
				disabled={submitting}
				class="w-full rounded-md bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50 md:hidden"
			>
				{submitting ? 'Memproses...' : 'Buat Pesanan'}
			</button>
		</form>

		<div class="order-1 h-fit rounded-xl border border-zinc-200 p-5 md:order-2 md:w-80">
			<h2 class="mb-4 text-sm font-semibold text-zinc-700">Ringkasan Pesanan</h2>
			<div class="max-h-64 space-y-3 overflow-y-auto">
				{#each cart.items as item (item.productId)}
					<div class="flex items-center gap-3">
						<div class="h-12 w-12 shrink-0 overflow-hidden rounded-md bg-zinc-100">
			...
			<button
				...
				form="checkout-form"
				disabled={submitting}
				class="mt-4 hidden w-full rounded-md bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50 md:block"
			>
				{submitting ? 'Memproses...' : 'Buat Pesanan'}
			</button>
		</div>
